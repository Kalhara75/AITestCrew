...
/// <summary>
/// HTTP surface for the <c>run_auth_refreshes</c> table. Mirrors the
/// pending-verification endpoints — agents post a request when they catch
/// <c>AuthRequiredException</c>, the dashboard polls active refreshes and
...
public static class AuthRefreshEndpoints
{
    public static RouteGroupBuilder MapAuthRefreshEndpoints(this RouteGroupBuilder group)
    {
            ...
            IRunQueueRepository queueRepo) =>
        {
            ...
            // Sending {surface: "WebMvc"} leaves Target empty and RecordingService falls
            // back to Blazor — that's why MVC refresh used to open the Blazor browser.
