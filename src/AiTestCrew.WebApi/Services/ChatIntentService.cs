...
public class ChatIntentService : IChatIntentService
{
    ...
    private static string BuildSystemPrompt(object catalog, ChatRequestContext? context)
    {
        ...
        return $$$"""
            ...
            - When the user asks to record actions for an objective that has Source="ImportedFromXray" and whose step list is empty (a placeholder imported from Xray), emit recordingKind: Record (NOT RecordVerification). The recorder automatically detects the imported placeholder by slug match and fills it in-place, preserving any post-steps. Use RecordVerification only when the user explicitly asks to add a verification post-step AFTER an already-recorded or already-filled step. Example: "record actions for the imported test case 'Network Tariff Code Search Screen Functionality'" -> { "recordingKind": "Record", "caseName": "Network Tariff Code Search Screen Functionality", ... }.
