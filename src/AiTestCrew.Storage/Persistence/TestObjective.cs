...
/// <summary>
/// A single test objective within a test set â€” corresponds to one user objective.
...
public class TestObjective
{
    ...
    // -- Xray import fields (additive -- null/empty defaults; existing data unaffected) --

    /// <summary>
    /// Jira Xray ticket key this objective was imported from (e.g. PROJ-1234).
    /// Null for objectives not created via the Xray importer.
    /// Multiple objectives may share the same key when a ticket decomposes into several objectives.
    /// </summary>
    public string? XrayTicketKey { get; set; }

    /// <summary>
    /// Stable slug for this decomposed slice within the Xray ticket.
    /// Idempotency key for re-imports: matching is by (XrayTicketKey, XrayObjectiveSlug).
    /// Null when XrayTicketKey is null.
    /// </summary>
    public string? XrayObjectiveSlug { get; set; }

    /// <summary>
    /// Precondition bullets lifted from the Xray Description Preconditions section.
    /// Shown as a read-only note panel in the editor above the step list.
    /// Usable on non-imported objectives too. Re-import replaces (not appends) this list.
    /// </summary>
    public List<string> Preconditions { get; set; } = [];

    /// <summary>
    /// Free-form text from the Xray Description Test Data section.
    /// Null when absent. Same editor treatment as Preconditions.
    /// </summary>
    public string? TestDataNotes { get; set; }

    /// <summary>
    /// API test step definitions â€” each is an individual HTTP request + assertions.
    /// Populated for API_REST / API_GraphQL targets.
    /// </summary>
    public List<ApiTestDefinition> ApiSteps { get; set; } = [];

    /// <summary>
    /// Web UI test step definitions â€” each is a self-contained UI test case with Playwright steps.
    /// Populated for UI_Web_* targets.
    /// </summary>
    public List<WebUiTestDefinition> WebUiSteps { get; set; } = [];

    /// <summary>
    /// Desktop UI test step definitions â€” each is a self-contained UI test case with FlaUI steps.
    /// Populated for UI_Desktop_* targets.
    /// </summary>
    public List<DesktopUiTestDefinition> DesktopUiSteps { get; set; } = [];

    /// <summary>
    /// aseXML generation test step definitions â€” each is a template id + user field values.
    /// Populated for AseXml_Generate targets.
    /// </summary>
    public List<AseXmlTestDefinition> AseXmlSteps { get; set; } = [];

    /// <summary>
    /// aseXML delivery test step definitions â€” template + user field values + endpoint code.
    ...
    /// <summary>
    /// Per-environment {{Token}} â†’ value overrides. Outer key = environment key
    ...
    /// with the value from the active environment's dictionary (lenient â€” unknown
    ...
    // â”€â”€ Backward compatibility for old v2 JSON with singular fields â”€â”€

    /// <summary>
    /// Old singular API definition â€” when deserialized from JSON, promotes into ApiSteps.
    ...
    /// <summary>
    /// Old singular Web UI definition â€” when deserialized from JSON, promotes into WebUiSteps.
    /// Not serialized back out.
    /// </summary>
    ...
    public IEnumerable<(string ParentKind, int ParentStepIndex, int PostStepIndex, VerificationStep PostStep)> EnumerateAllPostSteps()
    {
        ...
        for (var i = 0; i < AseXmlDeliverySteps.Count; i++)
        {
            ...
            // Legacy field â€” still canonical for delivery parents until Slice 2
