...
REQ-024 added two further catalog fields sourced from `TemplateRegistry` (loaded from `templates/asexml/**/*.manifest.json` at startup): `aseXmlTransactionTypes` (alphabetically-sorted deduplicated array of every distinct `TransactionType` across loaded templates — the answer to "what aseXML / MIL / B2B transaction types are supported?") and `aseXmlTemplates` (one entry per template with `templateId`, `transactionType`, `transactionGroup`, `description`, and `userFields` — only the `source: "user"` fields from the manifest, stripped of auto-generated and constant fields). Both fields are `[]` when no templates are loaded, giving the LLM an unambiguous signal. The system prompt's "aseXML template catalog rules" block disambiguates "MIL endpoints" (`catalog.endpointsByEnv` — per-env Bravo DB rows) from "MIL transaction types" (`catalog.aseXmlTransactionTypes` — global template-registry entries) to prevent conflation.
