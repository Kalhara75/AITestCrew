...
| `src/AiTestCrew.Agents/Environment/RemoteEnvironmentResolver.cs` | Agent-side resolver wrapper that fetches DB + Service Bus connection strings from the server when local config is empty (REQ-021 proxy pattern) |
| `src/AiTestCrew.WebApi/Endpoints/AgentConfigEndpoints.cs` | Server endpoints for agent connection-string resolution: `GET /api/environments/{envKey}/connections/db/{key}` + `/servicebus/{key}` |
...
- `TestObjective` — one per user objective, contains `ApiSteps: List<ApiTestDefinition>`, `WebUiSteps: List<WebUiTestDefinition>`, `DesktopUiSteps: List<DesktopUiTestDefinition>`, `AseXmlSteps: List<AseXmlTestDefinition>`, and `AseXmlDeliverySteps: List<AseXmlDeliveryTestDefinition>`. Each `AseXmlDeliveryTestDefinition` can own `PostDeliveryVerifications: List<VerificationStep>` — recorded UI steps that run after delivery with `{{Token}}` substitution from the render context. `Source` field tracks origin: `"Generated"` (AI), `"Recorded"` (user recording), `"ImportedFromXray"` (REQ-017 importer), `"ImportedFromXray+Recorded"` (REQ-020 filled placeholder), or `"Generated+Recorded"` (REQ-023 AI objective extended by recording). Rebaseline is only allowed for `"Generated"` objectives — the gate checks `Source != "Generated"` (strict equality) so all hybrid/recorded variants are blocked. Multi-env fields: `AllowedEnvironments: List<string>` (empty = default-env only; otherwise restricts to the listed env keys) and `EnvironmentParameters: Dictionary<envKey, Dictionary<token, value>>` (per-env `{{Token}}` values applied at playback).
...
| Adding a new user role (beyond User / AuthSteward / Admin) | Manual -- three files | Add the new role string to the valid-roles set in `UserEndpoints.cs`; update the `IsVisibleToUser` switch in `AuthHealthEndpoints.cs`; update the role comment in `User.cs`. If new admin-level endpoints are needed, add a role check mirroring the existing Admin guard pattern. |
| Promoting a user to Admin or AuthSteward | Admin UI or curl | `PUT /api/users/{id}/role` with body `{"role":"AuthSteward"}` (or "Admin"). Caller must be Admin. The first-ever user is auto-promoted to Admin on account creation. |
| Adding a new user role (beyond User / AuthSteward / Admin) | Manual — new `AuthSurface` enum value + agent override | Add to `AuthSurface` in `Core/Models/Enums.cs`; override `Surface` + `TryRecoverFromLoginRedirectAsync` on the new agent; plumb through `AuthHealthEndpoints` if pre-flight is needed. |
...
| A new DB connection (e.g. SDR Reporting DB) | Manual — config-only | `Environments.<key>.DbConnections.<connectionKey>` (or top-level `TestEnvironment.DbConnections.<connectionKey>` fallback). Zero code changes. Agents running in remote mode (REQ-021) fetch the connection string from the server automatically â no config change needed on agent machines. |
